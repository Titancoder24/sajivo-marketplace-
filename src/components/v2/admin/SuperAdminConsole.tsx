...
import {
  ...
  ShieldCheck, TicketCheck, UsersRound, Plus,
} from "lucide-react";
...
function KnowledgePanel({ data, selected, onSelect, onRefresh }: { data: Overview; selected: Overview["knowledge"][number] | null; onSelect: (article: Overview["knowledge"][number] | null) => void; onRefresh: () => Promise<void> }) {
  const createArticle = () => onSelect({ id: "new", slug: "", title: "", category: "support", locale: "en", summary: "", body: "", keywords: [], status: "draft", version: 1, updated_at: new Date().toISOString() });
  return <div className="grid gap-5 xl:grid-cols-[340px_minmax(0,1fr)]"><section className="rounded-md border border-[#dce1dd] bg-white"><header className="flex items-center justify-between gap-3 border-b border-[#e4e8e5] p-4"><div><h2 className="text-sm font-extrabold">Angel datasets</h2><p className="mt-1 text-[10px] text-[#76807b]">Versioned knowledge supplied to Angel</p></div><button type="button" onClick={createArticle} className="inline-flex h-9 items-center gap-1.5 rounded-md bg-[#1d2a27] px-3 text-[10px] font-bold text-white"><Plus size={13}/>New dataset</button></header><div className="divide-y divide-[#ecefeb]">{data.knowledge.map((article) => <button key={article.id} onClick={() => onSelect(article)} className={`w-full px-4 py-4 text-left ${selected?.id === article.id ? "bg-[#eef3ef]" : "hover:bg-[#f7f9f7]"}`}><div className="flex items-center justify-between gap-2"><p className="truncate text-xs font-extrabold">{article.title}</p><Status value={article.status} /></div><p className="mt-2 text-[10px] text-[#75807a]">{article.category} · v{article.version} · {article.locale.toUpperCase()}</p></button>)}</div></section><section className="rounded-md border border-[#dce1dd] bg-white">{selected ? <ArticleEditor article={selected} onSaved={async () => { await onRefresh(); onSelect(null); }} /> : <div className="grid min-h-[560px] place-items-center p-8 text-center"><div><BookOpenCheck className="mx-auto text-[#90a098]" size={34} /><h2 className="mt-4 font-extrabold">Select or add a dataset</h2><p className="mt-2 text-sm text-[#76807b]">Paste business context, review it, and publish it to Angel.</p></div></div>}</section></div>;
}
...
function ArticleEditor({ article, onSaved }: { article: Overview["knowledge"][number]; onSaved: () => Promise<void> }) {
  ...
  async function save(event: FormEvent<HTMLFormElement>) { event.preventDefault(); setSaving(true); const form = new FormData(event.currentTarget); const response = await fetch(isNew ? "/api/v2/admin/knowledge" : `/api/v2/admin/knowledge/${article.id}`, { method: isNew ? "POST" : "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ title: form.get("title"), category: form.get("category"), locale: form.get("locale"), summary: form.get("summary"), body: form.get("body"), status: form.get("status"), keywords: String(form.get("keywords") || "").split(",").map((item) => item.trim()).filter(Boolean) }) }); const payload = await response.json().catch(() => ({})); setSaving(false); if (!response.ok) { toast.error(payload.error || "Dataset could not be saved"); return; } toast.success(isNew ? "Angel dataset created" : "Knowledge article updated"); await onSaved(); }
  const input = "mt-1 w-full rounded-md border border-[#d5dcd7] bg-white px-3 text-xs outline-none focus:border-[#829189]";
  return <form key={article.id} onSubmit={save} className="p-5 sm:p-6"><div className="flex flex-wrap items-center justify-between gap-3 border-b border-[#e7eae8] pb-5"><div><p className="text-[10px] font-extrabold uppercase tracking-[0.12em] text-[#d65f45]">Knowledge editor</p><h2 className="mt-1 text-lg font-extrabold">{isNew ? "Paste a new Angel dataset" : article.title}</h2></div><span className="text-[10px] text-[#78827d]">{isNew ? "New draft" : `Version ${article.version}`}</span></div><div className="mt-5 grid gap-4 sm:grid-cols-2"><label className="text-[10px] font-bold sm:col-span-2">Title<input name="title" defaultValue={article.title} required placeholder="What should Angel know?" className={`${input} h-10`} /></label><label className="text-[10px] font-bold">Category<select name="category" defaultValue={article.category} className={`${input} h-10`}><option>account</option><option>projects</option><option>subscriptions</option><option>credits</option><option>payments</option><option>communication</option><option>security</option><option>support</option></select></label><label className="text-[10px] font-bold">Language<select name="locale" defaultValue={article.locale} className={`${input} h-10`}><option value="en">English</option><option value="hi">Hindi</option></select></label><label className="text-[10px] font-bold sm:col-span-2">Answer summary<textarea name="summary" defaultValue={article.summary} required minLength={10} maxLength={500} rows={2} placeholder="A concise answer Angel can use first." className={`${input} resize-none p-3`} /></label><label className="text-[10px] font-bold sm:col-span-2">Business context / dataset<textarea name="body" defaultValue={article.body} required minLength={20} maxLength={12000} rows={12} placeholder="Paste the approved Sajivo process, policy, product information, or support guidance here…" className={`${input} resize-y p-3 leading-5`} /></label><label className="text-[10px] font-bold sm:col-span-2">Search keywords<input name="keywords" defaultValue={article.keywords.join(", ")} placeholder="project brief, proposal, milestone" className={`${input} h-10`} /></label><label className="text-[10px] font-bold">Status<select name="status" defaultValue={article.status} className={`${input} h-10`}><option value="draft">Draft</option><option value="published">Publish to Angel</option>{!isNew ? <option value="archived">Archived</option> : null}</select></label></div><div className="mt-6 flex justify-end"><button disabled={saving} className="flex h-10 items-center gap-2 rounded-md bg-[#1d2a27] px-4 text-xs font-bold text-white shadow-[0_3px_0_#0c1311] disabled:opacity-50">{saving ? <Loader2 className="animate-spin" size={15} /> : <CheckCircle2 size={15} />}{isNew ? "Create dataset" : "Save article"}</button></div></form>;
}
