...
export async function POST(request: Request) {
  ...
  const [{ data: profile }, { data: projects }, { data: wallet }, { data: subscription }, { data: payments }, { data: articles }, { data: history }] = await Promise.all([
    ...
    auth.supabase.from("ai_knowledge_articles").select("slug, title, category, summary, body, keywords").eq("status", "published").eq("locale", body.locale === "hi" ? "hi" : "en").limit(50),
  ...
  const selectedArticles = rankKnowledge(articles ?? [], message);
  ...
  try {
    const completion = await requestAngelCompletion([
